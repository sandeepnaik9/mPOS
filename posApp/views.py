...
def manage_employees(request):
    ...
    if request.method == 'GET':
        ...
        if 'id' in data:
            id= data['id']
        if id.isnumeric() and int(id) > 0:
            employee = Employees.objects.filter(code=id).first()
...
def save_employee(request):
    ...
    if len(check) > 0 :
        ...
        try:
            if (data['code']).isnumeric() and int(data['code']) > 0 :
                save_product = Employees.objects.filter(code = data['code']).update(code=data['code'], fullname=data['name'], address = data['address'], salary = float(data['salary']),status = data['status'],dob=data['dob'])
            else:
                save_product = Employees(code=data['code'], fullname=data['name'], address = data['address'], salary = float(data['salary']),status = data['status'],dob=data['dob'])
                save_product.save()
            resp['status'] = 'success'
            messages.success(request, 'Product Successfully saved.')
        except Exception as e:
            print(e)
            resp['status'] = 'failed'+str(e)
    return HttpResponse(json.dumps(resp), content_type="application/json")
